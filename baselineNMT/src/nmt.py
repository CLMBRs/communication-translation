...
def valid_bleu(valid_labels, model, args, tt, dir_dic, which_dataset="valid"):
    ...
    print(bleu.find("="))
    print(bleu.find(",", bleu.find("=") + 1))
    print(bleu[103])
    print(bleu[103:112])
    print(bleu[bleu.find("=") + 1:bleu.find(",", bleu.find("=") + 1)])
    bleu_score = float(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--proj", type=int, default=False, help="Which GPU to run"
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        "--valid_batch_size", type=int, default=128, help="Batch size"
    )
    ...
    print(
        "mean length train src:",
        sum([len(s[0]) for s in train_org_src_]) / len(train_org_src_)
    )
    print(
        "mean length train trg:",
        sum([len(s[0]) for s in train_org_trg_]) / len(train_org_trg_)
    )
    ...
    print(
        "{} EN images, {} {} images".format(
            len(train_org_src), len(train_org_trg), args.trg.upper()
        )
    )
