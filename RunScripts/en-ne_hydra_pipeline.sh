...
OUTPUT_DIR=Output/en-ne_pipeline/bt_sec_${EX_ABBR}
...
python -u -m EC_finetune +ec=${CAPTIONS_CONFIG} \
    ec/language=${LANG} \
    ec/data=clipL \
    ec.output_dir=Output/en-ne_pipeline/captions_${EX_ABBR} \
    ec.model.model_name=Output/en-ne_pipeline/bt_init/last \
    ec.model.freeze_sender=True \

# Do EC
python -u -m EC_finetune  +ec=${EC_CONFIG} \
    ec/language=${LANG} \
    ec.output_dir=Output/en-ne_pipeline/ec_${EX_ABBR} \
    ec.model.model_name=Output/en-ne_pipeline/captions_${EX_ABBR} \
