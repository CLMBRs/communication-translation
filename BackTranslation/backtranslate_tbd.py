...
def validation(args, model, tokenizer, source_meta, target_meta, save_filename_suffix=None):
    ...
    for i, batch in enumerate(
        ...
        assert len(translated_ids) == len(source_string_batch) == len(reference_batch)
...
def main(args, backtranslation_pack):
    ...
    for step in range(args.num_steps):
        ...
        if args.do_validation and step % args.validate_every == 0:
            ...
            if best_val < ave_val_score or isinf(best_val):
                ...
                validation(
                    args, source2target_model, tokenizer, source_meta,
                    target_meta, save_filename_suffix="best.val"
                )
                validation(
                    args, target2source_model, tokenizer, target_meta,
                    source_meta, save_filename_suffix="best.val"
                )
                save_model(
    ...
    validation(
        args, source2target_model, tokenizer, source_meta,
        target_meta, save_filename_suffix="last.val"
    )
    validation(
        args, target2source_model, tokenizer, target_meta,
        source_meta, save_filename_suffix="last.val"
    )
