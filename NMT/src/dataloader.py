...
def next_batch_joint(images, labels, batch_size, num_dist, tt):
    ...
    for batch_idx in range(batch_size):
        ...
        num_caps = [
            len(labels[label]) for label in img_indices
        ]  # (num_dist) number of captions for each image
        cap_indices = [
            random.randint(0, num_cap) for num_cap in num_caps
        ]  # (num_dist)
        lsn_cap = [
            labels[img_idx][cap_idx]
            for img_idx, cap_idx in zip(img_indices, cap_indices)
        ]  # (num_dist, 2048)
    ...
    spk_imgs, spk_caps, lsn_imgs, lsn_caps, whichs = spk_imgs[
        sorted_order], spk_caps[sorted_order], lsn_imgs[sorted_order], lsn_caps[
            sorted_order], whichs[sorted_order]
    ...
    spk_caps_in = [
        np.lib.pad(
            cap, (0, seq_len - len(cap)), 'constant', constant_values=(0, 0)
        ) for cap in spk_caps_in
    ]
    ...
    return (
        spk_imgs, lsn_imgs, spk_caps_in, spk_cap_lens, lsn_caps, total_indices,
        spk_caps_out, whichs
    )
...
def next_batch_naka_enc(images, lab_org, batch_size, tt):
    image_ids = random.choice(
        range(len(lab_org)), batch_size, replace=False
    )  # (num_dist)
    caption_ids = [
        random.randint(0, len(lab_org[image_ids[idx]]))
        for idx in range(batch_size)
    ]  # choose an object
    captions = np.array(
        [
            lab_org[image_id][caption_id]
            for (image_id, caption_id) in zip(image_ids, caption_ids)
        ]
    )

    caption_lens = [
        (idx, len(x), image_id, caption_id)
        for idx, (x, image_id, caption_id
                 ) in enumerate(zip(captions, image_ids, caption_ids))
    ]
    ...
    caps_in = [
        np.lib.pad(
            cap, (0, max(caps_in_lens) - ln),
            'constant',
            constant_values=(0, 0)
        ) for (cap, ln) in zip(caps_in, caps_in_lens)
    ]
    ...
    caps_mid = [
        np.lib.pad(
            cap, (0, max(caps_mid_lens) - ln),
            'constant',
            constant_values=(0, 0)
        ) for (cap, ln) in zip(caps_mid, caps_mid_lens)
    ]
    ...
    imgs = Variable(
        torch.index_select(images, 0, torch.LongTensor(image_ids)),
        requires_grad=False
    )
