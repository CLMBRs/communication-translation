...
class NMT(torch.nn.Module):
    ...
    def forward(
        self, src_caps_in, src_caps_in_lens, trg_sorted_idx, trg_caps_in,
        trg_caps_in_lens
    ):
        enc_hid = self.encoder(
            src_caps_in, src_caps_in_lens
        )  # (batch_size, D_hid)
        ...
        dec_logits, _ = self.decoder(
            enc_hid, trg_caps_in, trg_caps_in_lens, "argmax"
        )
        return dec_logits
    ...
    def return_h(self, pretrained_dict):
        ...
        for item in pretrained_dict.keys():
            diag_h[item] = torch.nn.Parameter(
                self.tt.FloatTensor(pretrained_dict[item].size()),
                requires_grad=True
            )
    ...
    def translate(self, src, args):
        ...
        src = Variable(
            self.tt.LongTensor(src), requires_grad=False
        )  # (batch_size, seq_len)
        ...
        if args.decode_how == "beam":
            gen_idx = self.decoder.beam_search(
                concept, args.beam_width, args.norm_pow
            )
...
class RnnListener(torch.nn.Module):
    def __init__(self, lang, args):
        ...
        if args.w2v:
            self.emb = nn.Embedding.from_pretrained(
                args.src_embed, freeze=True, padding_idx=0
            )
        ...
        self.hid_to_hid = nn.Linear(
            args.num_directions['lsn'][lang] * args.D_hid, args.D_hid
        )
        self.drop = nn.Dropout(p=args.dropout)
    ...
    def forward(self, spk_msg, spk_msg_lens):
        ...
        pack = torch.nn.utils.rnn.pack_padded_sequence(
            spk_msg_emb, spk_msg_lens, batch_first=True
        )
...
class Speaker(torch.nn.Module):
    def __init__(self, lang, args):
        ...
        self.rnn = nn.GRU(
            args.D_emb,
            args.D_hid,
            args.num_layers['spk'][lang],
            batch_first=True
        )

        if args.w2v:
            self.emb = nn.Embedding.from_pretrained(
                args.trg_embed, freeze=True, padding_idx=0
            )
        else:
            self.emb = nn.Embedding.from_pretrained(
                args.trg_embed, freeze=True, padding_idx=0
    ...
    def forward(self, h_img, caps_in, caps_in_lens, sample_how):
        ...
        pack = torch.nn.utils.rnn.pack_padded_sequence(
            caps_in_emb, caps_in_lens, batch_first=True
        )
        ...
        unpacked_logits = self.hid_to_voc(
            unpacked.contiguous().view(-1, self.D_hid)
        )
        ...
        if sample_how == "argmax":
            ...
            _, comm_label = gumbel_softmax(
                unpacked_logits, self.temp, self.hard, self.tt
            )  # (batch_size, num_cat)

        comm_label = comm_label.view(batch_size, seq_len)
        len_scat = [list(range(x)) + [0] * (seq_len - x) for x in caps_in_lens]
        mask = Variable(
            self.tt.LongTensor(batch_size, seq_len).zero_().scatter_(
                1, self.tt.LongTensor(len_scat), 1
            )
        )
    ...
    def sample(self, h_img, argmax):
        ...
        h_img = h_img.unsqueeze(0).view(1, -1, self.D_hid).repeat(
            self.num_layers, 1, 1
        )
        ...
        for idx in range(self.seq_len):
            ...
            if np.array_equal(
                done, np.array([True for x in range(batch_size)])
            ):
                break
    ...
    def beam_search(self, h_ctx, width, norm_pow):  # # out (batch_size, D_hid)
        ...
        ft = self.tt.LongTensor(
            [self.w2i["<BOS>"] for ii in range(batch_size)]
        )[:, None]
        input = self.emb(
            Variable(ft)
        )  #.to("cuda:0") ) # (batch_size, 1, D_emb)
        hid = h_ctx[None, :, :].repeat(
            self.num_layers, 1, 1
        ...
        for tidx in range(self.seq_len):
            ...
            pre_prob = self.tt.FloatTensor([[x[0] for x in ee] for ee in live]
                                          ).view(batch_size, -1, 1)
            ...
            for bidx in range(batch_size):
                ...
                if num_live > 0:
                    ...
                    for eidx, (topi, topv) in enumerate(
                        zip(tis, tvs)
                    ):  # NOTE max width times
            ...
            if num_dead == [width for ii in range(batch_size)]:
                break
            in_vocab_idx = [
                [x[2] % voc_size for x in ee] for ee in live
            ]  # NOTE batch_size first
            input = self.emb(
                Variable(self.tt.LongTensor(in_vocab_idx)).view(-1)
            ).view(-1, 1, self.D_emb)
            bb = 1 if tidx == 0 else width
            in_width_idx = [
                [x[2] / voc_size + bbidx * bb for x in ee]
        ...
        dead_ = [
            [(float(a) / math.pow(len(b), norm_pow), b, c) for (a, b, c) in ee]
            for ee in dead
        ]
        ans = []
        for dd_ in dead_:
