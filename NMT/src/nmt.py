...
def translate(args, agent, labels, i2w, batch_size, which, tt):
    ...
    for idx in range(len(en_hyp)):
        ...
        print(
            u"{} ref {} | {}".format(
                args.trg.upper(), idx + 1, en_ref[idx].strip()
            )
        )
...
def valid_bleu(valid_labels, model, args, tt, dir_dic, which_dataset="valid"):
    ...
    destination = dir_dic["path_dir"] + "{}_hyp_{}".format(
        which_dataset, args.decode_how
    )
    ...
    if (args.src == "en" and
        args.trg == "de") or (args.src == "de" and args.trg == "en"):
        command = 'perl {}/multi-bleu.perl {} < {}'.format(
            scr_path(), '{}/ref/{}_many_{}'.format(
                dir_dic["data_path"], args.trg, which_dataset
            ), destination
        )
    elif (args.src == "en" and
          args.trg == "cs") or (args.src == "cs" and args.trg == "en"):
        command = 'perl {}/multi-bleu.perl {} < {}'.format(
            scr_path(), '{}/ref_encs/{}_many_{}'.format(
                dir_dic["data_path"], args.trg, which_dataset
            ), destination
        )
    elif (args.src == "en" and
          args.trg == "ro") or (args.src == "ro" and args.trg == "en"):
        command = 'perl {}/multi-bleu.perl {} < {}'.format(
            scr_path(), '{}/ref_enro/{}_many_{}'.format(
                dir_dic["data_path"], args.trg, which_dataset
            ), destination
        )
    elif (args.src == "en" and
          args.trg == "fr") or (args.src == "fr" and args.trg == "en"):
        command = 'perl {}/multi-bleu.perl {} < {}'.format(
            scr_path(), '{}/ref_enfr/{}_many_{}'.format(
                dir_dic["data_path"], args.trg, which_dataset
            ), destination
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--w2v", type=int, default=False, help="Which GPU to run"
    )
    parser.add_argument(
        "--dif_loss", type=int, default=True, help="Which GPU to run"
    )

    parser.add_argument(
        "--proj", type=int, default=True, help="Which GPU to run"
    )
    parser.add_argument(
        "--dataset",
        type=str,  # NOTE beam / greedy
    ...
    parser.add_argument(
        "--decode_how",
        type=str,
        default="beam",  # NOTE beam / greedy
        help="Which GPU to run"
    )
    parser.add_argument(
        "--beam_width", type=int, default=12, help="Which GPU to run"
    )
    parser.add_argument(
        "--norm_pow", type=float, default=0.0, help="Which GPU to run"
    )
    ...
    parser.add_argument(
        "--D_hid", type=int, default=512, help="Token embedding dimensionality"
    )
    parser.add_argument(
        "--D_emb", type=int, default=256, help="Token embedding dimensionality"
    )
    ...
    parser.add_argument(
        "--dropout", type=float, default=0.2, help="Dropout keep probability"
    )

    parser.add_argument(
        "--temp", type=float, default=1.0, help="Gumbel temperature"
    )
    parser.add_argument(
        "--hard",
        action="store_true",
    ...
    parser.add_argument(
        "--print_every", type=int, default=100, help="Save model output."
    )
    parser.add_argument(
        "--valid_every",
        type=int,
    ...
    parser.add_argument(
        "--save_every", type=int, default=2000, help="Save model output."
    )
    ...
    if (args.src == "en" and
        args.trg == "de") or (args.src == "de" and args.trg == "en"):
        (w2i_src, i2w_src, w2i_trg, i2w_trg) = [
            torch.load(data_path + 'dics/{}'.format(x))
            for x in "{}_w2i {}_i2w {}_w2i {}_i2w".
            format(args.src, args.src, args.trg, args.trg).split()
        ]
    elif (args.src == "en" and
          args.trg == "cs") or (args.src == "cs" and args.trg == "en"):
        (w2i_src, i2w_src, w2i_trg, i2w_trg) = [
            torch.load(data_path + 'dics_encs/{}'.format(x))
            for x in "{}_w2i {}_i2w {}_w2i {}_i2w".
            format(args.src, args.src, args.trg, args.trg).split()
        ]
    elif (args.src == "en" and
          args.trg == "ro") or (args.src == "ro" and args.trg == "en"):
        (w2i_src, i2w_src, w2i_trg, i2w_trg) = [
            torch.load(data_path + 'dics_enro/{}'.format(x))
            for x in "{}_w2i {}_i2w {}_w2i {}_i2w".
            format(args.src, args.src, args.trg, args.trg).split()
        ]
    elif (args.src == "en" and
          args.trg == "fr") or (args.src == "fr" and args.trg == "en"):
        (w2i_src, i2w_src, w2i_trg, i2w_trg) = [
            torch.load(data_path + 'dics_enfr/{}'.format(x))
            for x in "{}_w2i {}_i2w {}_w2i {}_i2w".
            format(args.src, args.src, args.trg, args.trg).split()
        ]
    else:
        (w2i_src, i2w_src, w2i_trg, i2w_trg) = [
            torch.load(data_path + 'dics_entr/{}'.format(x))
            for x in "{}_w2i {}_i2w {}_w2i {}_i2w".
            format(args.src, args.src, args.trg, args.trg).split()
        ]
    ...
    print(
        "mean length valid src:",
        sum([len(s[0]) for s in valid_org_src]) / len(valid_org_src)
    )
    print(
        "mean length test src:",
        sum([len(s[0]) for s in test_org_src]) / len(test_org_src)
    )
    print(
        "mean length valid trg:",
        sum([len(s[0]) for s in valid_org_trg]) / len(valid_org_trg)
    )
    print(
        "mean length test trg:",
        sum([len(s[0]) for s in test_org_trg]) / len(test_org_trg)
    )
    ...
    hyperparam_str = "lr_{}.dropout_{}.D_hid_{}.D_emb_{}.vocab_size_{}_{}/".format(
        args.lr, args.dropout, args.D_hid, args.D_emb, args.vocab_size_src,
        args.vocab_size_trg
    )
    path_dir = path + model_str + hyperparam_str
    [
        check_mkdir(xx) for xx in
        [saved_results_path() + task_path, path, path + model_str, path_dir]
    ]
    ...
    print(
        "{} EN images, {} {} images".format(
            len(train_org_src), len(train_org_trg), args.l2.upper()
        )
    )
    ...
    pretrained_dict = {
        'decoder.rnn.weight_ih_l0':
            model_trg_dict['speaker.rnn.weight_ih_l0'].cuda(args.gpuid),
        'decoder.rnn.weight_hh_l0':
            model_trg_dict['speaker.rnn.weight_hh_l0'].cuda(args.gpuid),
        'decoder.rnn.bias_ih_l0':
            model_trg_dict['speaker.rnn.bias_ih_l0'].cuda(args.gpuid),
        'decoder.rnn.bias_hh_l0':
            model_trg_dict['speaker.rnn.bias_hh_l0'].cuda(args.gpuid),
        'decoder.hid_to_voc.weight':
            model_trg_dict['speaker.hid_to_voc.weight'].cuda(args.gpuid),
        'decoder.hid_to_voc.bias':
            model_trg_dict['speaker.hid_to_voc.bias'].cuda(args.gpuid),
        'encoder.rnn.weight_ih_l0':
            model_src_dict['listener.rnn.weight_ih_l0'].cuda(args.gpuid),
        'encoder.rnn.weight_hh_l0':
            model_src_dict['listener.rnn.weight_hh_l0'].cuda(args.gpuid),
        'encoder.rnn.bias_ih_l0':
            model_src_dict['listener.rnn.bias_ih_l0'].cuda(args.gpuid),
        'encoder.rnn.bias_hh_l0':
            model_src_dict['listener.rnn.bias_hh_l0'].cuda(args.gpuid),
        'encoder.hid_to_hid.weight':
            model_src_dict['listener.hid_to_hid.weight'].cuda(args.gpuid),
        'encoder.hid_to_hid.bias':
            model_src_dict['listener.hid_to_hid.bias'].cuda(args.gpuid)
    }
    ...
    train_loss_dict = {
        "loss": AverageMeter(),
        "acc": AverageMeter(),
        "loss_seq": AverageMeter(),
        "loss_aux": AverageMeter()
    }
    for epoch in range(args.num_games):
        loss = forward_nmt(
            train_labels, model, train_loss_dict, args, loss_fn, tt, epoch
        )
        optimizer.zero_grad()
        loss.backward()
        ...
        if epoch % args.print_every == 0:
            print(
                "epoch {} | train {:.3f} norm {:.3f} | loss_seq {:.3f} | loss_aux {:.9f}"
                .format(
                    epoch, train_loss_dict['loss'].avg, total_norm,
                    train_loss_dict['loss_seq'].avg,
                    train_loss_dict['loss_aux'].avg
                )
            )
        ...
        if epoch % args.valid_every == 0:
            ...
            for idx in range(args.print_every):
                _ = forward_nmt(
                    valid_labels, model, valid_loss_dict, args, loss_fn, tt,
                    epoch
                )

            cur_val_loss = valid_loss_dict['loss'].avg
            bleu_score = valid_bleu(
                valid_labels, model, args, tt, dir_dic, "valid"
            )
            ...
            print(
                "epoch {} | valid {:.3f} bleu {}".format(
                    epoch, cur_val_loss, bleu_score
                )
            )
            ...
            if bleu_score > best_bleu["valid"]:
                ...
                test_bleu = valid_bleu(
                    test_labels, model, args, tt, dir_dic, "test"
                )
                best_bleu["test"] = test_bleu
            else:
                if epoch - best_epoch >= args.stop_after * args.valid_every:
                    print(
                        "Validation BLEU not improving after {} iterations, early stopping"
                        .format(args.stop_after * args.valid_every)
                    )
    ...
    result.write(
        "{}\t{}\t{}\n".format(
            best_bleu['valid'], best_bleu['test'], hyperparam_str
        )
    )
    result.close()
