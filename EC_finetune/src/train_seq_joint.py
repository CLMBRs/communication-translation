...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--len_loss", type=int, default=False, help="Which GPU to run"
    )
    parser.add_argument(
        "--vocab_size",
        type=int,
        default=
        4035,  #The EC vocab_size should be in line with the vocab_size in NMT fine-tuning. 
        help="EC vocab size"
    )
    parser.add_argument(
        "--alpha", type=float, default=1.0, help="Which GPU to run"
    )
    parser.add_argument(
        "--two_fc",
        action="store_true",
    ...
    parser.add_argument(
        "--D_img", type=int, default=2048, help="ResNet feature dimensionality"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--print_every", type=int, default=50, help="Save model output."
    )
    parser.add_argument(
        "--ECemb", type=int, default=5000, help="Set The EC Embedding Size"
    )
    parser.add_argument(
        "--valid_every",
        type=int,
    ...
    parser.add_argument(
        "--save_every", type=int, default=4000, help="Save model output."
    )
    ...
    model_str = "fixed_{}.learned_{}.{}_loss/".format(
        fixed, learned, args.which_loss
    ...
    path = "{}sentence_level/{}/joint_model/".format(
        saved_results_path(), task_path
    )
    hyperparam_str = "{}_dropout_{}.alpha_{}.lr_{}.temp_{}.D_hid_{}.D_emb_{}.num_dist_{}.vocab_size_{}_{}.hard_{}/".format(
        mill, args.dropout, args.alpha, args.lr, args.temp, args.D_hid,
        args.D_emb, args.num_dist, args.vocab_size, args.vocab_size, args.hard
    )
        ...
        with torch.no_grad():
            ...
            if epoch % args.valid_every == 0:
                ...
                if float(s_new.split()[-6][:-2]) > 99.0:
                    path_model = open(
                        path_dir + "model_{}_{}_{}.pt".format(
                            float(s_new.split()[-6][:-2]), epoch,
                            args.vocab_size
                        ), "wb"
                    )
