...
class BartGumbelEncoder(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            attention_mask = invert_mask(attention_mask)
        inputs_embeds = input_embeds if input_embeds is not None else self.embed_tokens(
            input_ids
        )
...
class BartDecoder(nn.Module):
    ...
    def forward(
        ...
        x = input_embeds if input_embeds is not None else self.embed_tokens(
            input_ids
        )
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def gumbel_generate(
        ...
        **model_kwargs
        ...
        if is_greedy_gen_mode:
            ...
            # greedy search
            return self.gumbel_greedy_search_new(
                input_ids,
            ...
            raise NotImplementedError(
                "Only greedy gumbel generation is implemented so far (num_beams must be 1)"
            )
        ...
        **model_kwargs
        ...
        if "lang_mask" in model_kwargs:
            ...
            invalid_token_ids = set(
                np.arange(len(lang_mask)
                         )[~torch.isfinite(lang_mask).cpu().numpy()]
            )
    ...
    def gumbel_greedy_search(
        ...
        **model_kwargs
        ...
        Return:
            ...
            batches finished early due to the :obj:`eos_token_id`.
        """
        generate_from_logits = (
            "generate_from_logits" in model_kwargs and
            model_kwargs["generate_from_logits"]
        )
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList(
        )
        max_length = max_length if max_length is not None else self.config.max_length
        ...
        # Get the one-hot vector for the padding token
        pad_token_onehot = (
            torch.arange(0, self.embed_tokens_size) == pad_token_id
        ).float().clone().detach()
        pad_token_onehot = pad_token_onehot.to(generated_token_ids.device)
        ...
        # Initialize the sequence of logits with the one-hot vector(s) for the starting tokens
        generated_logits = (
            torch.arange(
        ...
        while cur_len < max_length:
            ...
            if generate_from_logits:
                model_inputs = self.prepare_inputs_for_generation(
                    generated_token_ids,
                    decoder_input_embeds=generated_embeds,
                    **model_kwargs
                )
            else:
                model_inputs = self.prepare_inputs_for_generation(
            ...
            # pre-process distribution
            adjusted_next_logits = logits_processor(
                generated_token_ids, adjusted_next_logits
            )
    ...
    def gumbel_greedy_search_new(
        ...
        **model_kwargs,
        ...
        pad_token_onehot = pad_token_onehot.to(input_ids.device)
        ...
        generated_logits = generated_logits.to(input_ids.device)
        generated_logits = [generated_logits]
        ...
        generated_samples = generated_samples.to(input_ids.device)
        generated_samples = [generated_samples]
        ...
        while True:
            ...
            if "lang_mask" in model_kwargs:
                # here, the place we don't want have value of -inf
                adjusted_next_logits = next_token_logits + model_kwargs[
                    "lang_mask"]
            ...
            # pre-process distribution
            adjusted_next_logits = logits_processor(
                input_ids, adjusted_next_logits
            )

            # argmax
            next_tokens = torch.argmax(adjusted_next_logits, dim=-1)
            next_samples = F.gumbel_softmax(
                adjusted_next_logits, tau=self.temp, hard=self.hard
            )
            if generate_from_logits:
            ...
            if eos_token_id is not None:
                ...
                next_token_logits = next_token_logits.T * unfinished_sequences + \
                (pad_token_onehot.unsqueeze(dim=1)) * (1 - unfinished_sequences)
                next_token_logits = next_token_logits.T
            ...
            generated_samples.append(next_samples)
            generated_sentence_len = (~(input_ids == pad_token_id)).sum(dim=1)
...
class BartForCausalLanguageModeling(PretrainedBartModel):
    def __init__(self, config: BartConfig):
        ...
        self.decoder = BartDecoder(config, self.shared)
        self.register_buffer(
            "final_logits_bias", torch.zeros((1, self.shared.num_embeddings))
        )
        self.init_weights()
